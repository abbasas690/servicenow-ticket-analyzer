class ServiceNowClient {
    ...
    for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
      try {
        ...
        if (res.status === 429 || res.status >= 500) {
          lastError = new Error(`Server ${res.status}, retrying (${attempt + 1}/${this.maxRetries})`);
          emit({ kind: "warn", status: res.status, attempt: attempt + 1 });
  ...
  async fetchAudit(sysIds, fieldNames, onProgress, signal, tableName = "incident") {
    ...
    if (Object.keys(byTicket).length || !sysIds.length) return byTicket;
    const probe = await this.#fetchAuditOne(sysIds[0], fieldNames, tableName, 1);
    if (!probe.length) {
      this.onDiagnostic?.({ kind: "warn", note: "audit empty for all tickets; no per-ticket fallback probe data either" });
      return byTicket;
    }
    ...
    return this.#fetchAuditPerTicket(sysIds, fieldNames, onProgress, signal, tableName);
  }

  async #fetchAuditBatched(sysIds, fieldNames, onProgress, signal, tableName) {
    if (this.auditInBlocked) {
      return this.#fetchAuditPerTicket(sysIds, fieldNames, onProgress, signal, tableName);
    }
    const byTicket = {};
    const batches = [];
