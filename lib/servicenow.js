class ServiceNowClient {
  ...
  async #fetchHistoryBatched(sysIds, fieldNames) {
    ...
    if (skipped) {
      this.onDiagnostic?.({
        kind: "warn",
        note: `${skipped} history lines lacked update_time and were excluded`
      });
    }
    return byTicket;
  }
  ...
  async #fetchViaActivity(sysIds, fieldNames, onProgress, signal, tableName) {
    const parse = typeof Analysis !== "undefined" ? Analysis?.extractEventsFromActivity : null;
    if (!parse) throw new Error("Activity parser module not loaded");
    ...
    for (const [idx, sysId] of sysIds.entries()) {
      ...
      try {
        ...
        throw new Error(
          `Activity stream unavailable (${err.message}) - switch Timeline source to 'sys_history_line only'`
        );
      }
