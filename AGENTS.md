...
| `analysis/aiextract.js` | Pure local-AI helpers: `AI_MODELS` catalog (3 downloadable browser models), `buildClosurePrompt(notes)` (chat messages demanding JSON-only reply), `parseClosureJson(text)` (tolerant: strips `<think>`/fences, first balanced `{...}`, alt keys, boolean `is_permanent`, regex fallback without braces) → `{solutionType:"Permanent fix"|"Workaround", rootCause}`. Attached to `globalThis.AiExtract` like other pure modules |
| `analysis/aiclient.js` | Page-side promise client for the AI worker (`createAiClient()` → `ensure(modelId,onProgress)`/`extract(notes)`/`dispose()`); spawns a module worker from `viewer/aiworker.js`, matches responses by id, rejects all on worker crash. Used by viewer AND settings so heavy inference never blocks page UI |
| `viewer/aiworker.js` | Module worker doing ALL transformers.js work off the main thread: lazy-imports the vendored runtime + aiextract, sets `numThreads=1` and local `wasmPaths`, caches one pipeline per modelId, answers `{type:"ensure"}` (weight download w/ progress) and `{type:"extract"}` (prompt→generate→parseClosureJson). Nulls its pipeline on any error so the next call reloads |
| `lib/vendor/transformers.min.js` | Vendored @huggingface/transformers v3 (ESM, dynamic-`import()`ed inside aiworker only) + the ONNX runtime pair `ort-wasm-simd-threaded.jsep.{mjs,wasm}` it loads. `env.backends.onnx.wasm.wasmPaths = chrome.runtime.getURL("lib/vendor/")` is REQUIRED in every caller — the default CDN path violates extension_pages CSP (`script-src 'self'`) with a cryptic "no available backend found". Model weights stream from huggingface.co on first download and cache in the origin Cache Storage — needs the HF/xethub host permissions AND `'wasm-unsafe-eval'` in CSP; ORT runs single-threaded (`numThreads=1`) because blob workers are CSP-blocked |
...
Regression-test the AI closure-note JSON parser with `node tools/ai-parse-test.js`.
