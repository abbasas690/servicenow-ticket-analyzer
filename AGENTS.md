...
| `analysis/phase2.js` | Pure functions `extractTimelines(auditRows, ctx)` and `analyzeAll(...)` implementing the four timestamp rules; `normalizeAuditRefs(byTicket, refPairs)` rewrites display-name group/member values to configured sys_ids (32-hex values and unknown strings pass through) for history-table sources |
