...
| `lib/vendor/transformers.min.js` | Vendored @huggingface/transformers v3 (ESM, dynamic-`import()`ed from viewer/settings). Model weights stream from huggingface.co on first download and cache in the origin Cache Storage — needs the HF/xethub/jsdelivr host permissions AND `'wasm-unsafe-eval'` in extension_pages CSP; ORT runs single-threaded (`numThreads=1`) because blob workers are CSP-blocked |
