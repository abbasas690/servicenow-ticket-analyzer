...
| `settings/settings.html|js` | Options page (`chrome.runtime.openOptionsPage`). Edits `chrome.storage.local.pluginSettings`: `{version:2, instanceUrl, defaults:{ticketType, queues:[name...], teamMembers:[name...]}, params:{tablePageSize, debugResponses}}`. Queues/members are one NAME per line (legacy `Name \| sys_id` lines are accepted on read — the `\| sys_id` tail is stripped; legacy `{name,sysId}` objects migrate to their `.name`). No server calls at all. No JSON export/import — storage only. Background applies params in makeClient |
